...
class NE40_vm(vrnetlab.VM):
    def __init__(self, username, password, hostname, conn_mode):
        ...
        super(NE40_vm, self).__init__(username, password, disk_image=disk_image, ram=2048, smp="2", driveif="virtio")
    ...
    def bootstrap_config(self):
        ...
        self.wait_write(cmd=f"local-user {self.username} password irreversible-cipher {self.password}", wait="]")
        self.wait_write(cmd=f"local-user {self.username} service-type ssh", wait="]")
        self.wait_write(cmd=f"local-user {self.username} user-group manage-ug", wait="]")
        self.wait_write(cmd="quit", wait="]")
        ...
        self.wait_write(cmd="quit", wait="]")
        self.wait_write(cmd=f"ssh user {self.username} authentication-type password ", wait="]")
    ...
    def startup_config(self):
        ...
        self.wait_write(cmd=f"ssh user {self.username} sftp-directory cfcard:", wait="]")
        ...
        ssh_client.connect(hostname="localhost", username=self.username, password=self.password)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
      "--connection-mode", default="tc", help="Connection mode to use in the datapth",
      )
